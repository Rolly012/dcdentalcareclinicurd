	...
	<body class="ttr-opened-sidebar ttr-pinned-sidebar" style="background-color: #FBFBFB;">
		...
		<main class="ttr-wrapper" style="background-color: #FBFBFB;">
			<div class="container-fluid">
				<div class="db-breadcrumb">
					<h4 class="breadcrumb-title">Manage Services</h4>
					<ul class="db-breadcrumb-list">
						<li><i class="ti-list"></i>Services</li>
					</ul>
				</div>  
				...
				<div class="row">
					<div class="col-lg-12 m-b30">
							...
							<div class="heading-bx left">
								<h2 class="m-b10 title-head">Services <span></span></h2>
							</div>
								...
								<hr>
								<div align="right">
									<a href="" class="btn green radius-xl" data-toggle="modal" data-target="#add-announcement" style="background-color: <?php echo $primary_color; ?>"><i class="ti-announcement"></i><span>&nbsp;ADD NEW SERVICE</span></a>&nbsp;&nbsp;<a href="archived-services" class="btn red radius-xl"><i class="ti-archive"></i><span>&nbsp;ARCHIVED SERVICES</span></a>
								</div>
								...
								<div id="add-announcement" class="modal fade" role="dialog">
									<form class="edit-profile m-b30" method="POST" enctype="multipart/form-data">
										<div class="modal-dialog modal-lg">
											<div class="modal-content">
												<div class="modal-header">
													<h4 class="modal-title"><img src="../assets/images/<?php echo $web_icon; ?>.png" style="width: 30px; height: 30px;">&nbsp;Add Service</h4>
													<button type="button" class="close" data-dismiss="modal">&times;</button>
												</div>
												<div class="modal-body">
													<div class="row">
		...
		<script type="text/javascript">
			$(document).ready(function() {
				...
				?>
				$('#content_<?php echo $read['id']; ?>').html("<?php echo str_replace(utf8_encode('"'), "'", substr($read['details'], 0, 150)); ?>..... <a id='read-more-<?php echo $read['id']; ?>' style='color: <?php echo $primary_color; ?>'>Read more</a>");

				$(document).on('click', '#read-more-<?php echo $read['id']; ?>', function() {
					var content = "<?php echo str_replace(utf8_encode('"'), "'", preg_replace('/[\r\n]+/i', " ", $read['details'])); ?> <a id='show-less-<?php echo $read['id']; ?>' style='color: <?php echo $primary_color; ?>'>Show less</a>";

					$('#content_<?php echo $read['id']; ?>').html(content);
				});

				$(document).on('click', '#show-less-<?php echo $read['id']; ?>', function() {
					$('#content_<?php echo $read['id']; ?>').html("<?php echo str_replace(utf8_encode('"'), "'", substr($read['details'], 0, 150)); ?>..... <a id='read-more-<?php echo $read['id']; ?>' style=' <?php echo $primary_color; ?>'>Read more</a>");
				});
				<?php
